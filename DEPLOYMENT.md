...
2. Confirm the build command is `npm install && npx prisma generate && npm run build`.
3. Confirm the start command is `npm run start`.
4. Run `npx prisma migrate deploy --schema prisma/schema.prisma` against the production database if migrations are pending.
5. Run `npx prisma db seed` once if the database is empty.
