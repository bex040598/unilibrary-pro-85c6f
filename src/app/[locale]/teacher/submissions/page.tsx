...
export default async function TeacherSubmissionsPage() {
  ...
  const resources = await prisma.resource.findMany({
    where: {
      ...
      status: {
        in: ["PENDING_REVIEW", "REJECTED", "NEEDS_REVISION", "APPROVED"]
      }
    },
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
      <div>
        <p className="text-sm uppercase tracking-[0.24em] text-primary">Submission tracking</p>
        <h1 className="mt-2 text-3xl font-semibold">Moderator ko‘rib chiqayotgan resurslar</h1>
      </div>
      <Card className="space-y-4">
        {resources.map((resource) => (
          <div key={resource.id} className="rounded-2xl border border-border bg-surface-soft p-4">
            <p className="font-medium">{resource.title}</p>
            <p className="mt-1 text-sm text-muted-foreground">{resource.status}</p>
